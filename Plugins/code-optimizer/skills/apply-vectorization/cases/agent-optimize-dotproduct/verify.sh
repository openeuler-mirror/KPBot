...
# baseline：禁掉 GCC 自动向量化，强制标量参考
gcc -O2 -fno-tree-vectorize -std=c11 -Isrc bench/main.c src/kernel.c -lm -o build/baseline
...
export REL_ERR="$(awk -F= '/^REL_ERR=/{print $2}' results/optimized.out)"
...
json.dump({
    ...
    "rel_err": float(os.environ["REL_ERR"] or 0),
