...
- gather_task_id = TaskCreate({ subject: "      └ 收集信息", description: "读取错误详情和项目上下文" })
- iterate_task_id = TaskCreate({ subject: "      └ 迭代修复", description: "分析错误 → 修复代码 → 编译验证 → 功能测试，最多5轮" })
- output_task_id = TaskCreate({ subject: "      └ 输出结果", description: "汇总修复过程，输出 JSON 契约" })
...
每轮迭代开始时：TaskUpdate({ taskId: iterate_task_id, status: "in_progress", activeForm: "迭代修复 - 第N轮" })（将 N 替换为当前轮次）
...
TaskUpdate({ taskId: iterate_task_id, status: "completed" })
