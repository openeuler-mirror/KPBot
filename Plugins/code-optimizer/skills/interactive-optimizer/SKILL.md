...
执行过程中用 TaskCreate 创建任务，实时反映当前进度。用户可随时查看 `/tasks` 了解优化进展。
...
**步骤**：
1. 从 `$ARGUMENTS` 或对话中获取用户意图。若信息不足，用 AskUserQuestion 收集：
   - 目标项目路径和构建方式
...
8. **联网兜底**：若分析过程中遇到不熟悉的代码模式或不确定瓶颈根因，用 WebSearch 搜索相关算法的 ARM 优化方案（如"<algorithm> NEON optimization"）和类似性能现象的根因分析。搜索结果可补充或修正本地 reference 的判断。
...
**每轮开始时**：创建本轮子任务：
```
TaskCreate: "Round N: 策略发现与展示"（activeForm: "分析第 N 轮优化策略"）
TaskCreate: "Round N: 逐策略实施"（activeForm: "实施第 N 轮优化"）
TaskCreate: "Round N: 重新分析与总结"（activeForm: "总结第 N 轮优化结果"）
```
设置依赖关系：策略发现 → 逐策略实施 → 重新分析。

将 "Round N: 策略发现与展示" 标记为 `in_progress`。

对当前热点函数，读取源码和反汇编，对照 `03-optimization-patterns.md` 第 2 节"代码特征→策略映射"找到匹配策略。按优先级排序（优先低风险高收益）。若本地 reference 中无匹配策略但代码有明显优化空间（如特殊算法、非标准模式），用 WebSearch 搜索补充策略（如"<算法名> optimization ARM NEON"），将搜索结果作为额外候选策略展示。对每个候选策略，**展示格式必须包含两部分证据**：
...
用 `AskUserQuestion` multiSelect 让用户从候选策略中选择本次要实施的策略：
...
优化方向修正循环（步骤 6a/7a）：
  ...
  b. 修正建议来源：
     ...
     — 询问用户是否有实现思路上的建议
     — **联网搜索**：当以上来源都无法定位问题时，用 WebSearch 搜索具体错误信息或优化模式，获取外部参考案例（见"联网搜索"章节触发场景表"优化方向修正"行）
```
...
| Read | 工具调用 | 读取源码和参考文件 |
| Edit | 工具调用 | 应用代码变更 |
| AskUserQuestion | 工具调用 | 策略选择和流程确认 |
| git add / commit / checkout | Bash | 版本控制（单策略单提交、失败回滚） |
| WebSearch | 工具调用 | 联网搜索优化技术、解决方案、参考案例 |
...
当本地知识库（4 个 reference 文件 + 指令查询脚本）无法覆盖当前问题时，**主动使用 WebSearch 和 WebFetch 获取外部知识**。这不是可选项——遇到不确定的问题时，联网搜索比凭记忆猜测更可靠。
...
1. **先搜后读**：先用 `WebSearch` 搜索关键词（中文/英文均可），获取相关页面列表。
...
- **策略选择**：AskUserQuestion multiSelect（Phase 3.2）
...
7. **知识缺口先搜索**：遇到本地 reference 和查询脚本无法解答的问题时（熟悉度 < 70%），优先用 WebSearch 获取外部信息，禁止凭记忆猜测。搜索结果写入 state 文件作为证据链。
...
对话过程中不输出 JSON 契约——所有交互通过自然语言、AskUserQuestion 和 Markdown 表格完成。
