...
```bash
bash .opencode/skills/kpbot-code-optimizer/scripts/print_logo.sh 2>/dev/null || bash "$HOME/.config/opencode/skills/kpbot-code-optimizer/scripts/print_logo.sh"
```
...
阶段分发分两种模式：**内联阶段**由主 Agent 直接执行，**子代理阶段**通过 Agent 工具分发为独立 subagent。
...
| 维度 | Agent tool（独立 subagent） | Skill tool（主线程） |
|------|-----------|-------------------|
...
```
❌ 对子代理阶段使用 Skill(skill="...") 直接在主线程加载   ← 无上下文隔离
❌ 对有用户交互的阶段使用 Agent 分发 subagent             ← 用户交互无法正确传递
```
...
流水线启动后，**首先通过 AskUserQuestion 选择运行模式**，决定后续所有用户交互环节的行为：
...
执行要求：
- 所有 `AskUserQuestion` 交互必须在提示文本中展示默认选项和超时时间，例如：`120 秒无输入将默认选择：继续`。
...
会话层恢复：
- 若 Claude 显示 `Churned for ...` / `※ recap:` 后停在输入提示，但尚未写出最终完成 artifact，外层 driver 应发送 `继续` 恢复执行。
- 该恢复动作记为 `recap_continue`，必须与普通空闲催促 `nudge_continue` 分开计数和汇报。
- 若已检测到 `optimization_reports/run_*/batch_result.json`、`SESSION_COMPLETE`、`BATCH_END`、`SESSION_END` 等完成标记，不得再发送 `recap_continue` 或 `nudge_continue`。

### 前置步骤：环境预检（Sandbox & Permissions）

流水线启动后，检测运行环境，判断 sandbox 是否已开启，若未开启则引导用户选择提效方案。

**检测方法**：读取配置文件中的 `sandbox` 字段（优先 `.claude/settings.local.json`，其次 `.claude/settings.json`）：

```bash
jq -r '.sandbox.enabled' .claude/settings.local.json 2>/dev/null | grep -q "true" && echo "SANDBOX_ENABLED"
jq -r '.sandbox.enabled' .claude/settings.json 2>/dev/null | grep -q "true" && echo "SANDBOX_ENABLED"
jq -r '.sandbox.autoAllowBashIfSandboxed' .claude/settings.local.json 2>/dev/null | grep -q "true" && echo "AUTO_ALLOW"
jq -r '.sandbox.autoAllowBashIfSandboxed' .claude/settings.json 2>/dev/null | grep -q "true" && echo "AUTO_ALLOW"
```

判定逻辑：

若 `context.pipeline_mode == "auto"` → **跳过环境预检**，直接进入 Phase 0（用户已在 bypass permissions 模式下运行，无需额外配置）。

| `enabled` | `autoAllowBashIfSandboxed` | 结果 |
|-----------|---------------------------|------|
| `true` | `true` | sandbox 已开启且 Bash 自动放行，跳过预检，直接进入 Phase 0 |
| `true` | `false` / 未设置 | sandbox 已开启但 Bash 仍需确认，转入**分支 B**（配置 permissions） |
| `false` / 未配置 | — | sandbox 未开启，调用 `AskUserQuestion`：

```json
{
  "questions": [{
    "question": "检测到 sandbox 未开启。Sandbox 模式可自动允许 Bash 命令调用，大幅减少工具确认次数。推荐以下提效方案：",
    "header": "环境预检",
    "multiSelect": false,
    "options": [
      {"label": "开启 sandbox（推荐）", "description": "在 .claude/settings.local.json 中启用 sandbox，重启会话后生效，自动允许所有 Bash 命令"},
      {"label": "配置 permissions", "description": "在 .claude/settings.local.json 中添加常用命令白名单（mkdir/git/grep/find/make 等），当前会话即可生效"},
      {"label": "跳过", "description": "不进行任何配置，直接开始流水线"}
    ]
  }]
}
```
...
**分支 A：用户选择"开启 sandbox"**

在 `.claude/settings.local.json` 中写入 sandbox 配置（与已有配置合并）：
...
然后告知用户：
> sandbox 配置已写入 .claude/settings.local.json。**请重启 Claude Code 会话**以使 sandbox 生效。重启后重新运行 `/kpbot-code-optimizer`。
>
> 若无法重启，可回复「继续」使用当前会话（无 sandbox，确认次数较多）。

若用户选择「继续」→ 进入 Phase 0。若用户选择重启 → 流水线终止，等待用户重启后重新运行。

**分支 B：用户选择"配置 permissions"**

将常用开发/构建/性能分析命令写入 `.claude/settings.local.json` 白名单（mkdir, git, grep, find, make, cmake, gcc, perf, python 等），格式为 `Bash(cmd:*)`。写入时与已有配置合并（去重），保留用户已有的 sandbox 等配置。配置写入后直接进入 Phase 0。

**分支 C：用户选择"跳过"** → 直接进入 Phase 0。
...
信息收集完成后，依次调用 TaskCreate 创建以下任务：

```
TaskCreate({ subject: "收集优化目标信息", description: "与用户交互确定优化类型、代码路径、测试用例", activeForm: "收集优化目标信息" })
TaskCreate({ subject: "准备项目环境", description: "验证项目状态、定位优化目标、建立性能基线", activeForm: "准备项目环境" })
TaskCreate({ subject: "优化轮次循环", description: "迭代式 profiling 驱动优化：每轮重新发现热点 → 分解子任务 → 应用优化，直到无新优化点或达到上限", activeForm: "优化轮次循环" })
```
...
1. `TaskUpdate({ taskId: <当前任务ID>, status: "in_progress" })`
2. 使用 Skill 工具加载对应 SKILL.md：
   ```
   Skill({ skill: "<skill-name>", args: "$ARGUMENTS" })
   ```
3. 收集 SKILL.md 输出中的 JSON 契约，更新 context
4. `TaskUpdate({ taskId: <当前任务ID>, status: "completed" })`
5. ★ 阶段总结与确认
...
4. 调用 Agent 工具分发 fresh subagent（**不可用 Skill 工具代替**）：
   ```
   Agent({
     description: "<当前阶段名称>",
     subagent_type: "general-purpose",
...
7. `TaskUpdate({ taskId: <当前任务ID>, status: "completed" })`
...
```
  ...
  e. 创建本轮任务组：
     round_task_id = TaskCreate({
       subject: "第 ${round} 轮优化",
       description: "重新 profiling 发现热点 → 分解子任务 → 应用优化",
...
```
  a. TaskUpdate({ taskId: <round_task_id>, status: "in_progress" })
  b. 执行 DecomposeTasks 阶段（内联）
     - 使用 Skill 工具加载：
       Skill({ skill: "decompose-tasks", args: "$ARGUMENTS" })
...
```
  d. 执行 AnalyzeTestcase 阶段（内联）
     - 使用 Skill 工具加载：
       Skill({ skill: "analyze-testcase", args: "$ARGUMENTS" })
...
**注意**：以下 TaskCreate 调用必须**逐个串行**创建，不要批量并行调用。

```
for sub_task in sub_tasks:
  parent_id = TaskCreate({
    subject: "SubTask #${sub_task.id}: ${sub_task.function}",
    description: "${sub_task.reason}"
  })

  analyze_id = TaskCreate({
    subject: "  └ 热点分析",
    description: "对 ${sub_task.function} 进行动静态分析，发现优化点",
    activeForm: "SubTask #${sub_task.id}: ${sub_task.function} - 热点分析"
  })
```
...
```
for sub_task in sub_tasks:
  a. TaskUpdate({ taskId: <parent_id>, status: "in_progress" })
     TaskUpdate({ taskId: <analyze_id>, status: "in_progress" })
  ...
  c. TaskUpdate({ taskId: <analyze_id>, status: "completed" })

  d. 检查 context.analyzeHotspot：
     - status == "empty" 且用户未补充 → TaskUpdate({ taskId: <parent_id>, status: "completed" })
       记录结论（无可优化点），继续下一个子任务
...
**auto 模式**：跳过 AskUserQuestion，直接使用 AI 发现的优化点（`all_points = optimization_points`），不补充、不勾选、不重做。

**collaboration 模式**：AnalyzeHotspot 阶段总结与确认时，AskUserQuestion 提供四个选项：
...
**collaboration 模式**：AnalyzeHotspot 完成后，通过 AskUserQuestion 询问用户是否启用 AnalyzeCallerContext 阶段，从调用者视角发现额外的优化机会。
...
```
  a. 调用 Agent 工具分发 fresh subagent：
     Agent({
...
```
  ...
  for opt_point in context.analyzeHotspot.optimization_points:
    ...
    # 逐个串行创建优化点子阶段任务
    opt_parent_id = TaskCreate({
      subject: "  └ 优化点 ${opt_point.id}: ${opt_point.type}",
      description: "优先级: ${opt_point.priority} | 信心度: ${opt_point.confidence} | 预期: ${opt_point.expected_speedup}",
      activeForm: "SubTask #${sub_task.id}: 优化点 ${opt_point.id} (${opt_point.type})"
    })
    decide_id = TaskCreate({
      subject: "    └ 策略确认",
      description: "确认 ${opt_point.type} 优化点的可行性和参数"
    })
    apply_id = TaskCreate({
      subject: "    └ 代码优化",
      description: "对 ${opt_point.type} 优化点应用代码变更"
    })
    challenge_id = TaskCreate({
      subject: "    └ 对抗性审核",
      description: "挑战所有优化判断——成功追问放弃的方案，失败追问做不到的理由"
    })
    verify_id = TaskCreate({
      subject: "    └ 验证",
      description: "验证 ${opt_point.type} 优化点的正确性和性能"
    })
    fix_id = TaskCreate({
      subject: "    └ 代码修复",
      description: "修复编译或功能测试错误"
    })
    re_verify_id = TaskCreate({
      subject: "    └ 重新验证",
      description: "修复后重新验证"
    })
```
...
```
    a. TaskUpdate({ taskId: <opt_parent_id>, status: "in_progress" })
    ...
    c. TaskUpdate({ taskId: <decide_id>, status: "completed" })
       检查 decideOptimization.status：
       - "skipped" → 标记后续 apply/verify/fix/re_verify 为 completed
       - TaskUpdate({ taskId: <opt_parent_id>, status: "completed" })
       - 记录结论到 optimization_point_results，继续下一个优化点

    d. TaskUpdate({ taskId: <apply_id>, status: "in_progress" })
    ...
    f. TaskUpdate({ taskId: <decide_id>, status: "completed" })
    ...
    ```
      ...
      while challenge_round < 3:
        ...
        a. TaskUpdate({ taskId: <challenge_id>, status: "in_progress" })
        b. 调用 Agent 工具分发 fresh subagent：
           Agent({
        ...
        d. TaskUpdate({ taskId: <challenge_id>, status: "completed" })
    ...
    g. TaskUpdate({ taskId: <apply_id>, status: "in_progress" })
    ...
    i. TaskUpdate({ taskId: <apply_id>, status: "completed" })
    ...
    k. TaskUpdate({ taskId: <verify_id>, status: "in_progress" })
    ...
    m. TaskUpdate({ taskId: <verify_id>, status: "completed" })
    ...
    n. 条件分支：检查 verify-optimization 结果
       ...
       **情况 1：verify status 为 "verified"/"marginal"/"unverified"**
       ...
       - TaskUpdate({ taskId: <re_verify_id>, status: "completed" })
       - TaskUpdate({ taskId: <opt_parent_id>, status: "completed" })
       - 记录结果，继续下一个优化点
    ...
    o. TaskUpdate({ taskId: <fix_id>, status: "in_progress" })
    p. 执行 FixCode 阶段
        ...
        - ★ 阶段总结与确认
    q. TaskUpdate({ taskId: <fix_id>, status: "completed" })
    ...
    r. 检查 fix-code 结果：
        ...
        **fix-code status == "fixed"**：
        ...
        - 若来自 apply-optimization 失败路径：先将 verify_id 设为 in_progress（跳过了 verify）
        - TaskUpdate({ taskId: <re_verify_id>, status: "in_progress" })
        - 重置 context.verifyOptimization = null
        - 递增 stage_versions.VerifyOptimization
        - 调用 Agent 工具分发 fresh subagent 执行 VerifyOptimization：
          Agent({
...
```
  全部优化点完成后：
  - TaskUpdate({ taskId: <parent_id>, status: "completed" })
...
| 函数级 AnalyzeHotspot | optimization_points 为空 | 记录结论，通过 AskUserQuestion 询问用户是否启用 AnalyzeCallerContext（如 folded 数据可用） |
...
**auto 模式**：跳过 AskUserQuestion，自动同意（等同于用户选择"同意，继续下一步"）。review criteria 不通过时的重试逻辑不变（最多 3 轮），重试耗尽后直接标记 BLOCKED（不询问用户是否放弃或强制继续）。

**collaboration 模式**：AskUserQuestion 提供"同意，继续下一步"和"不同意，需要重做"两个选项，展示阶段核心要点、总结文件路径和超时默认值：`120 秒无输入将默认选择：同意，继续下一步`。
