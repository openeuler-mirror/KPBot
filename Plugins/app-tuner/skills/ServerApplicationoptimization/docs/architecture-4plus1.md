...
本文档是 `server-application-optimization` skill 的设计入口，用 4+1 架构视图描述使用场景、模块分层、运行机制、仓库组织和平台装载方式。当前公共行为已按“服务器应用优化 Agent”架构图重构：用户界面输入与确认 → 环境诊断和备份 → 性能采集与瓶颈识别 → 根据采集信息生成候选优化 skill 列表 → 迭代验证 → 报告、review、环境还原和案例归档。
...
- 主实现以 `skills/server-application-optimization/SKILL.md` 为准。
- 架构图映射以 `skills/server-application-optimization/references/application-agent-architecture.md` 为准。
- 细节契约以 `skills/server-application-optimization/references/` 为准。
- 专项能力以 `skills/server-application-optimization/subskills/` 为准。
- `ref-skills/` 只作为仓库内置外部能力源，由统一入口子 skill 条件接入。
## 2. 架构设计原则

- 单一主源：`skills/server-application-optimization/` 是唯一事实来源。
...
- 多平台接入：`.claude/`、`.opencode/`、`.agents/` 是轻量发现入口；Codex、Cursor 等编程 Agent 可直接加载主源目录，逻辑仍回到 `skills/server-application-optimization/`。
...
```
server-application-optimization 4+1 视图
├── 用例视图
│   ├── 客户端调用方式
│   ├── 整体编排调用
│   ├── 子 skill 独立调用
...
| 其他编程 Agent | 支持目录式 `SKILL.md` 或可读取仓库规则 | 指向 `skills/server-application-optimization/` 主源 |

各平台的轻量入口均为薄跳转层，最终加载同一主源 `skills/server-application-optimization/SKILL.md`，行为一致。
...
本轮重点验收 `server-application-optimization` 主框架，子 skill 由专项开发者后续迭代。主框架验收范围包括：
...
skills/server-application-optimization/   # [唯一主源]
  SKILL.md                          # 主编排入口
  agents/openai.yaml
...
.claude/skills/server-application-optimization/SKILL.md   # Claude Code 轻量入口
.opencode/skills/server-application-optimization/SKILL.md # OpenCode 轻量入口
.agents/skills/server-application-optimization/SKILL.md   # 通用 Agent 轻量入口

# 轻量入口职责：发现路径 + 名称一致 + 提示加载主源。轻量包装（非软链接），不复制实现。
# Codex、Cursor 等可直接加载 skills/server-application-optimization/ 主源目录。
...
仓库 (Repository)
  │
  │
  ├── Claude 入口:   .claude/skills/server-application-optimization/SKILL.md
  │     ├── ► Claude Code (发现加载)
  │     └── (薄跳转) ──► 主源
  │
  ├── OpenCode 入口: .opencode/skills/server-application-optimization/SKILL.md
  │     ├── ► OpenCode (发现加载)
  │     └── (薄跳转) ──► 主源
  │
  ├── Agent 入口:    .agents/skills/server-application-optimization/SKILL.md
  │     ├── ► Generic Agent (发现加载)
  │     └── (薄跳转) ──► 主源
  │
  └── 直接主源:      skills/server-application-optimization/
...
`server-application-optimization` 当前架构是“主编排 skill + references 契约 + 多专项 subskill + 仓库内置 ref-skill + 工具脚本 + 标准报告”的可扩展优化编排框架。
