...
```bash
python3 .claude/skills/batch-drive-optimize-pipeline/scripts/batch_drive_optimize.py \
...
`final.patch` is a clean source/config patch. It is assembled from an allowlist of tracked source, header, assembly, and build-system/config files, and excludes build outputs, perf data, compressed/test payloads, binaries, `.claude/**`, helper files, and copied optimization report artifacts from committed, working-tree, and untracked changes.
...
python3 .claude/skills/batch-drive-optimize-pipeline/scripts/batch_drive_optimize.py --monitor --out "$out"
```
