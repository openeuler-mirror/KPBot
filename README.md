...
> **注意**：OpenCode 兼容性修复位于 `feat/opencode-compatibility` 分支（尚未合入 main）。

```bash
git clone -b feat/opencode-compatibility https://gitcode.com/KPBot/KPBot.git
# 已 clone 的仓库：git checkout feat/opencode-compatibility && git pull
```
...
# 全局安装
/path/to/KPBot/init.sh global opencode

# 启动
opencode
```

安装脚本自动完成：Skills 软链接 -> AGENTS.md 系统提示词 -> 健康检查。
...
# 本地开发安装
claude plugins marketplace add /path/to/KPBot
...
安装后直接在 OpenCode 中用自然语言描述你的优化需求：
...
全局安装时 `init.sh` 会自动将相对路径重写为绝对路径。如果 KPBot 仓库移动了位置，需要重新安装：
...
├── plugins/
│   └── code-optimizer/            # 代码优化插件 (36 skills，源数据)
│       ├── .claude-plugin/
│       │   └── plugin.json
│       ├── hooks/                 #   SessionStart 欢迎横幅 hook
│       └── skills/                #   技能源目录
├── init.sh                        # Claude Code 安装脚本
├── CLAUDE.md
├── README.md
└── LICENSE
```
...
1. 在 `plugins/` 下创建新目录
2. 添加 `.claude-plugin/plugin.json`（Claude Code 清单）
3. 在 `skills/` 下创建技能目录（每个技能一个 `SKILL.md`）
4. 在 marketplace.json 的 `plugins` 数组中添加条目
5. 提交 PR
