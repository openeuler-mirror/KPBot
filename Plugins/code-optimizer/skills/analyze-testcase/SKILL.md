...
- analyze_task_id = TaskCreate({ subject: "    └ 多维分析", description: "收集源码 + 六维度分析（规模/并发/访存/缓存/预热/用例冲突）" })
- profile_task_id = TaskCreate({ subject: "    └ 生成性能画像", description: "生成性能画像 + 置信度评分 + 优化指导建议" })
