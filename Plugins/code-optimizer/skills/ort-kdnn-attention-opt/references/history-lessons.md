# 历史教训:被证明无效/放弃的方向(含数字)

读它的时机:准备尝试下列任一方向**之前**。每条都附带实测数字和原因——先人已经付过学费。

## 1. flash / online-softmax 系

| 方向 | 结果 | 原因 |
|---|---|---|
| flash 用于 Sq=1(decode) | 无意义 | attention 矩阵只有一行,没有可省的中间矩阵;rescaling 纯开销 |
...
的特例)。也就是说:**做 no-pack 拿到了 flash 的内存收益,避开了它的调度代价**——decode 场景就该这么选。
...
- KV cache 内置:由上层把 past+current 拼成 `[B,Sk,hidden]` 再进来;(评估结论:Model4 场景 KV cache
  上限 ~10%,vLLM 不可用,未做)
...
对策(已固化为本 skill 的协议):可观测性先行(dispatch/路径计数)→ sequential+inter=1 固定口径 →
交替多轮 + CV 门槛 → 单算子与端到端双数字并报。

## 6. KDNN MHA API 的生态对标(设计新算子接口时参考)

- oneDNN / XNNPACK / MLAS **均无** fused MHA/SDPA primitive——CPU 生态普遍留给上层拼,提供 fused
  CPU MHA 是少数派选择,也是收益来源。
