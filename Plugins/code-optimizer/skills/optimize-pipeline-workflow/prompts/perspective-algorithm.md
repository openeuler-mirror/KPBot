...
2. **知识检索**：优先 WebSearch/WebFetch 检索学术文献和开源实现，不可用时降级为本地知识库 + AI 自身训练数据
...
```
WebSearch: "<algorithm name> ARM aarch64 NEON SVE optimization"
WebSearch: "<algorithm name> SIMD acceleration paper"
WebSearch: "<algorithm name> fast algorithm alternative"
WebSearch: "<algorithm name> parallel algorithm"
```

对搜索结果中的高价值链接，用 WebFetch 获取详细内容。

#### 2b. 本地知识库（WebSearch 不可用时）

WebSearch/WebFetch 不可用（企业内网、离线环境等）时：
...
| `empty` | 未找到任何可替代方案 |
| `degraded` | WebSearch 不可用，仅使用知识库/AI 知识，结果可能不完整 |

**`search_summary.fallback_source`**：`"knowledge_base"` 或 `"ai_inference"` 或 `null`（WebSearch 成功时）。
