...
- route_task_id = TaskCreate({ subject: "      └ 策略路由", description: "判断策略类型，路由到对应优化子 Skill" })
- optimize_task_id = TaskCreate({ subject: "      └ 执行优化", description: "构造请求并调用优化子 Skill 生成代码变更" })
- compile_task_id = TaskCreate({ subject: "      └ 编译验证", description: "轻量快速编译检查，fail fast" })
- output_task_id = TaskCreate({ subject: "      └ 输出结果", description: "收集结果，输出 JSON 契约" })

### 步骤 0：判断策略类型

TaskUpdate({ taskId: route_task_id, status: "in_progress" })
...
策略路由完成：TaskUpdate({ taskId: route_task_id, status: "completed" })
进入对应优化步骤：TaskUpdate({ taskId: optimize_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: optimize_task_id, status: "completed" })
