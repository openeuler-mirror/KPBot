...
redfish_get() {
  ...
  local http_code
  http_code="$(curl -k -sS --connect-timeout 10 --max-time 30 \
    -u "${BMC_USER}:${BMC_PASS}" \
  ...
  echo "${http_code}" > "${OUTPUT_DIR}/.redfish_last_http_code"

  if [[ "${http_code}" == "200" ]]; then
    log_manifest "OK  redfish:${endpoint} -> ${output_file} (HTTP ${http_code})"
    return 0
  fi

  log_manifest "FAIL redfish:${endpoint} -> ${output_file} (HTTP ${http_code})"
  return 1
}
...
collect_bios() {
  ...
  if [[ -f "${OUTPUT_DIR}/.redfish_last_http_code" && "$(cat "${OUTPUT_DIR}/.redfish_last_http_code")" != "200" ]]; then
    local code
    code="$(cat "${OUTPUT_DIR}/.redfish_last_http_code")"
    printf 'redfish_auth_failed=true\n' >> "${redfish_file}"
    printf 'http_code=%s\n' "${code}" >> "${redfish_file}"
    if [[ "${code}" == "000" ]]; then
      printf 'reason=BMC 不可达（网络不通或 BMC 服务未启动）\n' >> "${redfish_file}"
    elif [[ "${code}" == "401" || "${code}" == "403" ]]; then
      printf 'reason=BMC 凭据错误或权限不足\n' >> "${redfish_file}"
    else
      printf 'reason=Redfish 采集失败（HTTP %s）\n' "${code}" >> "${redfish_file}"
    fi
    return
  fi
