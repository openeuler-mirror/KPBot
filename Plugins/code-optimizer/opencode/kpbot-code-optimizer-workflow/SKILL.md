---
name: kpbot-code-optimizer-workflow
description: 鲲鹏性能优化流水线（Workflow 版）— 与 kpbot-code-optimizer 功能等价。Claude Code 环境下优化核心循环委托给 Workflow 工具执行（确定性编排、结构化输出验证、并行执行）；OpenCode 环境下自动使用 task() subagent 模式（功能等价）。当用户想要优化 C/C++ 项目性能时触发。
---
...
**与 `kpbot-code-optimizer` 的区别**：本版本将 Phase 3 的优化核心循环（函数级 AnalyzeHotspot → 优化点级 Decide→Challenge→Apply→Verify→Fix）委托给 Workflow 工具执行。Workflow 提供 `pipeline()`/`parallel()` 确定性编排、结构化 JSON Schema 验证和函数级并行分析。用户交互、环境预检、DecomposeTasks、报告写入仍由 Orchestrator 直接处理。

若 Workflow 不可用，自动回退到 task subagent 模式（与 `kpbot-code-optimizer` 行为一致）。

## 架构

```
kpbot-code-optimizer-workflow (Orchestrator)           ← 主 Agent
  ...
  │    │   └── 逐函数 task() 分发（与 kpbot-code-optimizer 一致）
  │    │
  │    └── 轮次汇总 → 继续/停止?
  │
  └── Phase 4: 多轮最终汇总
...
五层分析模型与 `kpbot-code-optimizer` 一致：轮次循环 → DecomposeTasks（用例级）→ AnalyzeTestcase（测试用例级）→ AnalyzeHotspot（函数级）→ 优化 Skill（优化点级）。
...
for stage in [analyze-hotspot, ..., fix-code]:
  template = read({ filePath: "skills/kpbot-code-optimizer/prompts/stage.<name>.md" })
...
# Step 2: 委托 Workflow
Workflow({
  scriptPath: "skills/kpbot-code-optimizer-workflow/workflows/optimization-round.js",
...
Workflow 不可用时使用，与原 `kpbot-code-optimizer` 行为完全一致：
...
与 `kpbot-code-optimizer` 完全相同的 stages 配置，详见原文件 `skills/kpbot-code-optimizer/SKILL.md` 的 Stages 配置章节。本文件不重复定义，所有 reviewCriteria 和 post-stage 逻辑保持一致。

## 分发逻辑

当收到用户请求时，按以下流程执行。Phase 2（准备阶段）和 Phase 4（最终汇总）与 `kpbot-code-optimizer` 完全一致，详见原文件。Phase 3 的核心变更如下。

### 前置步骤 0：模式选择

与 `kpbot-code-optimizer` 完全一致：通过 `question` 选择 `collaboration` 或 `auto` 模式，存入 `context.pipeline_mode`。
...
与 `kpbot-code-optimizer` 完全一致：生成 `run_id`，准备进入 Phase 2。

### Phase 1：创建任务列表

与 `kpbot-code-optimizer` 完全一致：通过 todowrite 创建阶段任务。

### Phase 2：准备阶段（只跑一次）

与 `kpbot-code-optimizer` 完全一致：GatherContext → ParseIntent → PrepareProject，全部由主 Agent 内联执行。
...
与 `kpbot-code-optimizer` 完全一致，主 Agent 内联执行：
...
```
a. 确定 prompt 根路径：
   prompt_root = "<pipeline_root>/skills/kpbot-code-optimizer-workflow/prompts"

   <pipeline_root> 解析：从 skills/kpbot-code-optimizer-workflow/SKILL.md 的真实路径向上两级。
...
c. 调用 Workflow 工具（⚠️ CRITICAL: args 必须是 JSON 对象，不是字符串）：
   ...
   workflow_result = Workflow({
     scriptPath: "<pipeline_root>/skills/kpbot-code-optimizer-workflow/workflows/optimization-round.js",
...
使用 `task()` subagent 分发，与 `kpbot-code-optimizer` 完全一致的 task-based 分发逻辑。
...
回退执行逻辑详见 `skills/kpbot-code-optimizer/SKILL.md` 的「函数级子任务循环」和「优化点级子循环」章节。核心流程：
...
**注意**：Subagent 回退模式下，AnalyzeCallerContext 和用户补充优化点的交互逻辑与 `kpbot-code-optimizer` 完全一致，但不享受 Workflow 的并行化和结构化验证。

#### 3.3 轮次收尾

与 `kpbot-code-optimizer` 完全一致：
...
与 `kpbot-code-optimizer` 完全一致：确保 working tree 干净。

### Phase 4：多轮最终汇总

与 `kpbot-code-optimizer` 完全一致。从 `context.round_results` 提取数据，按 `skills/kpbot-code-optimizer/prompts/partials/final-summary.md` 格式汇总。
...
完整变量映射表详见 `skills/kpbot-code-optimizer/SKILL.md` 的「模板变量填充规则」章节，两者一致。
...
Subagent 回退模式下，与 `kpbot-code-optimizer` 的内联 review 完全一致。
...
| 资源 | 路径 | 用途 |
|------|------|------|
| Workflow Prompt 文件 | `skills/kpbot-code-optimizer-workflow/prompts/*.md` | 预清洗的完整 agent prompt（模板+SKILL.md 合并，去除 todowrite/skill tool/question） |
| 最终汇总格式 | `skills/kpbot-code-optimizer/prompts/partials/final-summary.md` | Phase 4 输出格式（复用原文件） |
...
| Workflow 脚本 | `skills/kpbot-code-optimizer-workflow/workflows/optimization-round.js` | 优化核心循环编排 |

## 与 kpbot-code-optimizer 的差异总结

| 维度 | kpbot-code-optimizer | kpbot-code-optimizer-workflow (Claude Code) | kpbot-code-optimizer-workflow (OpenCode) |
|------|------------------|------------------------------------------|---------------------------------------|
| Phase 3 优化执行 | 逐函数 task() 分发 | Workflow 委托（pipeline + parallel） | 逐函数 task() 分发（同 kpbot-code-optimizer） |
