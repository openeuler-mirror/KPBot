...
# 安装后目标项目侧（非插件源内目录）：
#   .opencode/skills/kpbot-app-tuner/   # OpenCode 安装目标（由 install.sh 生成）
#   ~/.claude/skills/kpbot-app-tuner/   # Claude Code 全局安装目标
#   ~/.config/opencode/skills/...       # OpenCode 全局安装目标
#
...
#           改动主流程语义 → 同步更新主源 SKILL.md、相关 reference、opencode 覆盖层、本文档。
```
...
仓库 (Repository)
  │
  │
  ├── 插件源覆盖层:   opencode/kpbot-app-tuner/SKILL.md
  │     └── install.sh 拷贝主源后，再用此文件覆盖 OpenCode 平台差异
  │
  ...
  └── 安装后目标（由 install.sh 在目标项目/用户目录生成）：
        .opencode/skills/kpbot-app-tuner/   # OpenCode 项目级安装
        ~/.config/opencode/skills/...       # OpenCode 全局安装
        ~/.claude/skills/kpbot-app-tuner/   # Claude Code 全局安装
        各入口均为薄跳转层，最终加载同一主源，行为一致。
```
...
- 内置 aarch64 全类别库检测是否可用（已合并到 `performance-library-selection`，依赖 `optimization_kb.json` 知识库可加载）。
