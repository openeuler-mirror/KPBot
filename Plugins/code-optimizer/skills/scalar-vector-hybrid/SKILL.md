...
- boundary_task_id = TaskCreate({ subject: "      └ 边界决策", description: "识别函数内标量/矢量的最优边界" })
- mapping_task_id = TaskCreate({ subject: "      └ 指令映射", description: "NEON→标量指令映射与开销分析" })
- codegen_task_id = TaskCreate({ subject: "      └ 代码生成", description: "生成标矢量混合代码" })
- verify_task_id = TaskCreate({ subject: "      └ 汇编验证", description: "反汇编验证标量/矢量边界正确性" })

### 步骤 1：读取源码并识别标量/矢量边界

TaskUpdate({ taskId: boundary_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: boundary_task_id, status: "completed" })

### 步骤 2：指令映射与开销分析

TaskUpdate({ taskId: mapping_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: mapping_task_id, status: "completed" })

### 步骤 3：生成标矢量混合代码

TaskUpdate({ taskId: codegen_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: codegen_task_id, status: "completed" })

### 步骤 4：汇编验证

TaskUpdate({ taskId: verify_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: verify_task_id, status: "completed" })
