...
description: 鲲鹏性能优化流水线（Workflow 版）— 与 kpbot-code-optimizer 功能等价，但优化核心循环委托给 Workflow 工具执行，提供确定性编排、结构化输出验证和并行执行能力。当用户想要优化 C/C++ 项目性能时触发。
---
...
若 Workflow 不可用，自动回退到 Agent subagent 模式（与 `kpbot-code-optimizer` 行为一致）。
...
kpbot-code-optimizer-workflow (Orchestrator)           ← 主 Agent
  ...
  │    │   └── 逐函数 Agent() 分发（与 kpbot-code-optimizer 一致）
  │    │
  │    └── 轮次汇总 → 继续/停止?
  │
  └── Phase 4: 多轮最终汇总
...
for stage in [analyze-hotspot, ..., fix-code]:
  template = Read("skills/kpbot-code-optimizer/prompts/stage.<name>.md")
...
与 `kpbot-code-optimizer` 完全一致：通过 `AskUserQuestion` 选择 `collaboration` 或 `auto` 模式，存入 `context.pipeline_mode`。

### 前置步骤：环境预检（Sandbox & Permissions）

与 `kpbot-code-optimizer` 完全一致：检测 sandbox 状态，引导用户选择配置方案。
...
与 `kpbot-code-optimizer` 完全一致：通过 TaskCreate 创建阶段任务。
...
当 Workflow 不可用时（`context.workflow_failed == true` 或 Workflow 工具不存在），使用与 `kpbot-code-optimizer` 完全一致的 Agent-based 分发逻辑。
...
| 资源 | 路径 | 用途 |
|------|------|------|
| Workflow Prompt 文件 | `skills/kpbot-code-optimizer-workflow/prompts/*.md` | 预清洗的完整 agent prompt（模板+SKILL.md 合并，去除 TaskCreate/Skill tool/AskUserQuestion） |
...
| 维度 | kpbot-code-optimizer | kpbot-code-optimizer-workflow |
|------|------------------|---------------------------|
