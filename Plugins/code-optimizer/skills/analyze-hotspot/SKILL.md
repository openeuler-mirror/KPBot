...
- dynamic_task_id = TaskCreate({ subject: "    └ 动态微架构分析", description: "perf stat/annotate/SPE 多维度动态分析" })
- static_task_id = TaskCreate({ subject: "    └ 静态代码扫描", description: "代码模式扫描，发现可优化模式" })
- points_task_id = TaskCreate({ subject: "    └ 生成优化点列表", description: "综合动静态分析结果，输出优化点" })

### 步骤 1：动态微架构分析

TaskUpdate({ taskId: dynamic_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: dynamic_task_id, status: "completed" })
TaskUpdate({ taskId: static_task_id, status: "in_progress" })
...
2. 先用 repo 内统一入口，且必须带 `--json`：
   ...
   ```
3. 如果当前 skill 通过 `~/.claude/skills/...` 软链接安装，先把 `CURRENT_SKILL_MD` 设为已加载的本 `SKILL.md` 文件路径，再解析真实路径：`skill_root="$(cd "$(dirname "$CURRENT_SKILL_MD")" && pwd -P)"`，再由 `skill_root/../..` 得到 `<pipeline_root>`。不要调用过期拷贝。
...
TaskUpdate({ taskId: static_task_id, status: "completed" })
TaskUpdate({ taskId: points_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: points_task_id, status: "completed" })
