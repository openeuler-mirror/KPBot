...
- profile_task_id = TaskCreate({ subject: "    └ 逐用例Profiling", description: "对每个测试用例执行 perf record 发现热点" })
- rank_task_id = TaskCreate({ subject: "    └ 跨用例综合排序", description: "加权汇总热点函数，生成优先级排序" })
- create_task_id = TaskCreate({ subject: "    └ 创建子任务", description: "生成函数级子任务列表" })

### 步骤 1：逐用例 Profiling

TaskUpdate({ taskId: profile_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: profile_task_id, status: "completed" })
TaskUpdate({ taskId: rank_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: rank_task_id, status: "completed" })
TaskUpdate({ taskId: create_task_id, status: "in_progress" })
...
TaskUpdate({ taskId: create_task_id, status: "completed" })
