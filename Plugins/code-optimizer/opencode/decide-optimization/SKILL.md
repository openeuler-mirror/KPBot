...
todowrite({
  todos: [
    { content: "验证与门控", status: "pending", priority: "high" },
    { content: "构造执行计划", status: "pending", priority: "high" }
  ]
})
...
todowrite({ todos: [{ content: "验证与门控", status: "completed", priority: "high" }] })
todowrite({ todos: [{ content: "构造执行计划", status: "pending", priority: "high" }] })
...
todowrite({ todos: [{ content: "构造执行计划", status: "completed", priority: "high" }] })
...
- `throughput-enhancement` 策略时需用户确认（question）
