...
| 工具/输入 | 用途 | 分级 | 缺失时处理 |
| --- | --- | --- | --- |
| `perf` | 热点采样 | 推荐 | 库替换路径降级 |
| `lsof` | 库依赖识别 | 推荐 | 仅保留有限静态判断 |
| `readelf` | 库/符号补充识别 | 推荐 | 降低识别覆盖率 |
| `subskills/performance-library-selection/references/optimization_kb.json` | 全类别库替换知识库（子 skill 内置） | 推荐 | aarch64 全类别检测路径阻断，回退通用经验路径 |
| `subskills/performance-library-selection/scripts/detect_all_libraries.sh` | 统一库类型识别 | 推荐 | 库类型识别降级 |
