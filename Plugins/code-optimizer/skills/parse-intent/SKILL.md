...
- collect_task_id = TaskCreate({ subject: "    └ 收集意图", description: "通过 AskUserQuestion 收集用户优化偏好" })
- synthesize_task_id = TaskCreate({ subject: "    └ 语义推断与合成", description: "关键词匹配 + 合成最终意图 + 设置平台约束" })

**重要**：本 Skill 所有用户交互必须通过 `AskUserQuestion` **工具**完成。**禁止**将问题以纯文本输出。

### 步骤 1：收集用户优化偏好

TaskUpdate({ taskId: collect_task_id, status: "in_progress" })

使用 AskUserQuestion 工具收集 2 个意图维度：
...
TaskUpdate({ taskId: collect_task_id, status: "completed" })
TaskUpdate({ taskId: synthesize_task_id, status: "in_progress" })
...
这两个维度不需要 AskUserQuestion，设置智能默认值：
...
TaskUpdate({ taskId: synthesize_task_id, status: "completed" })
...
- **必须使用 AskUserQuestion 工具**：所有用户交互通过 `AskUserQuestion` 工具完成，不可用纯文本替代
