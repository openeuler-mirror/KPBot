...
cp -r KPBot/Plugins/app-tuner/skills/kpbot-app-tuner ~/.claude/skills/
```
...
- Codex：在仓库上下文中直接使用 `Plugins/app-tuner/skills/kpbot-app-tuner/` 主源目录。
- Cursor：通过项目规则、Agent 配置或上下文指向 `Plugins/app-tuner/skills/kpbot-app-tuner/SKILL.md`。
- OpenCode：使用 `Plugins/app-tuner/opencode/kpbot-app-tuner/` 覆盖层（install 后落到目标项目 `.opencode/skills/`）。
- 其他支持目录式 `SKILL.md` 的 Agent：直接加载 `Plugins/app-tuner/skills/kpbot-app-tuner/`。
